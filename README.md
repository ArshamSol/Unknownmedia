...
- `js/data.js` — your content: work items, teaching, client work, about, contact
- `js/app.js` — view rendering + hash-based routing (`#teaching`, `#about`, `#work-project-one`, ...)

## Layout

- **Left rail** — brand/logo, tag filters, section nav (Portfolio / Teaching / Client Work / About / Contact) pinned to the bottom
- **List column** — scrollable numbered index of portfolio items, filtered by the active tag
- **Main pane** — renders the current view: a masonry grid on Portfolio, timeline rows on Teaching/Client Work, bio + CV on About, contact links on Contact
...
1. Edit `js/data.js` — that's the only file you need to touch to change content (work items, teaching, client work, bio, contact links).
